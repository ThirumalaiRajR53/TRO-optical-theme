...
document.querySelectorAll('[id^="Details-"] summary').forEach((t) => {
  t.setAttribute("role", "button"),
    ...
    t.closest("header-drawer") || t.parentElement.addEventListener("keyup", onKeyUpEscape);
});
  ...
  removeTrapFocus(),
    ...
    (trapFocusHandlers.keydown = function (e) {
      "TAB" === e.code.toUpperCase() &&
        ...
        (e.target === t || e.target === s) && e.shiftKey && (e.preventDefault(), r.focus()));
    }),
  ...
  validateQtyRules() {
    ...
    if (this.input.max) {
      let s = parseInt(this.input.max),
        r = this.querySelector(".quantity__button[name='plus']");
      r.classList.toggle("disabled", t >= s);
    }
  }
}
...
customElements.define("quantity-input", QuantityInput),
  ...
  (Shopify.setSelectorByValue = function (t, e) {
    for (var i = 0, s = t.options.length; i < s; i++) {
      var r = t.options[i];
      if (e == r.value || e == r.innerHTML) return (t.selectedIndex = i), i;
    }
  }),
  ...
  (Shopify.postLink = function (t, e) {
    ...
    for (var n in (r.setAttribute("method", i), r.setAttribute("action", t), s)) {
      var a = document.createElement("input");
      a.setAttribute("type", "hidden"), a.setAttribute("name", n), a.setAttribute("value", s[n]), r.appendChild(a);
    }
    ...
    countryHandler: function (t) {
      ...
      if ((this.clearOptions(this.provinceEl), i && 0 == i.length)) this.provinceContainer.style.display = "none";
        ...
        for (var s = 0; s < i.length; s++) {
          ...
          (e.value = i[s][0]), (e.innerHTML = i[s][1]), this.provinceEl.appendChild(e);
        }
...
class MenuDrawer extends HTMLElement {
  ...
  bindEvents() {
    try {
      this.querySelectorAll("summary").forEach((t) => {
        t.className.includes("sorting-summary") || t.addEventListener("click", this.onSummaryClick.bind(this));
      }),
        ...
        this.querySelector(".menu-drawer__header span:first-child").addEventListener("click", (t) => {
          window.innerWidth > "640" ||
            (document.querySelector(".utility-bar") && (document.querySelector(".utility-bar").style.display = "block"),
            this.closeMenuDrawer(t, this.mainDetailsToggle.querySelector("summary")),
            this.querySelector(".header__hamburger").setAttribute("aria-expanded", !1));
        }),
        this.querySelector(".header__hamburger").addEventListener("click", () => {
          window.innerWidth > "640" || (document.querySelector(".utility-bar").style.display = "none");
        });
    } catch (t) {}
  }
  onKeyUp(t) {
  ...
  onSummaryClick(t) {
    ...
    i === this.mainDetailsToggle
      ? (r && t.preventDefault(),
        r ? this.closeMenuDrawer(t, e) : this.openMenuDrawer(e),
      ...
      : setTimeout(() => {
          i.classList.add("menu-opening"),
            ...
            !n || n.matches ? a() : e.nextElementSibling.addEventListener("transitionend", a);
        }, 100);
  }
  openMenuDrawer(t) {
    setTimeout(() => {
...
class SlideshowComponent extends SliderComponent {
  ...
  focusOutHandling(t) {
    ...
    } else this.reducedMotion.matches || this.play();
  }
  focusInHandling(t) {
    if (this.sliderAutoplayButton) {
      let e = t.target === this.sliderAutoplayButton || this.sliderAutoplayButton.contains(t.target);
      e && this.autoplayButtonIsSetToPlay ? this.play() : this.autoplayButtonIsSetToPlay && this.pause();
    } else this.querySelector(".announcement-bar-slider").contains(t.target) && this.pause();
  }
  play() {
...
class VariantSelects extends HTMLElement {
  ...
  updateMedia() {
    ...
    let t = document.querySelectorAll(`[id^="MediaGallery-${this.dataset.section}"]`);
    t.forEach((t) => t.setActiveMedia(`${this.dataset.section}-${this.currentVariant.featured_media.id}`, !0));
    let e = document.querySelector(`#ProductModal-${this.dataset.section} .product-media-modal__content`);
    if (!e) return;
    let i = e.querySelector(`[data-media-id="${this.currentVariant.featured_media.id}"]`);
    e.prepend(i);
  }
  updateURL() {
    this.currentVariant &&
  ...
  updateVariantStatuses() {
    let t = this.variantData.filter((t) => this.querySelector(":checked").value === t.option1),
      e = [...this.querySelectorAll(".product-form__input")];
    e.forEach((i, s) => {
      if (0 === s) return;
      let r = [...i.querySelectorAll('input[type="radio"], option')],
        n = e[s - 1].querySelector(":checked").value,
        a = t.filter((t) => t.available && t[`option${s}`] === n).map((t) => t[`option${s + 1}`]);
      this.setInputAvailability(r, a);
    });
  }
  ...
  renderProductInfo() {
    ...
    fetch(
      ...
      .then((i) => {
        ...
        n && r && (r.innerHTML = n.innerHTML),
          l && d && (d.innerHTML = l.innerHTML),
          a &&
            o &&
            ((o.innerHTML = a.innerHTML),
            o.classList.toggle("visibility-hidden", a.classList.contains("visibility-hidden")));
        let u = document.getElementById(`price-${this.dataset.section}`);
        u && u.classList.remove("visibility-hidden"), d && d.classList.toggle("visibility-hidden", "" === l.innerText);
        let h = s.getElementById(`ProductSubmitButton-${e}`);
        this.toggleAddButton(!h || h.hasAttribute("disabled"), window.variantStrings.soldOut),
  ...
  toggleAddButton(t = !0, e, i = !0) {
    ...
    if (
      r &&
      (t
        ? (r.setAttribute("disabled", "disabled"), e && (n.textContent = e))
        : (r.removeAttribute("disabled"), (n.textContent = window.variantStrings.addToCart)),
      !i)
    )
      return;
  }
  setUnavailable() {
    let t = document.getElementById(`product-form-${this.dataset.section}`),
      ...
      r = document.getElementById(`Inventory-${this.dataset.section}`),
